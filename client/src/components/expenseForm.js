...
const ExpenseForm = () => {
  ...
  async function countExpenses() {
    const response = await axios.post(
      "http://13.53.182.149:4000/expense/countExpense",
      { rows },
  ...
  async function leaderboardFunc() {
    const response = await axios.get(
      "http://13.53.182.149:4000/purchase/leaderboard"
    );
  ...
  const paginatingExpenses = async () => {
    const res = await axios.post(
      `http://13.53.182.149:4000/expense/pagination/${page}`,
      { rows },
      { headers: { Authorization: token } }
    );
  ...
  const deleteExpense = async (id) => {
    await axios.get(`http://13.53.182.149:4000/expense/delete-expense/${id}`, {
      headers: { Authorization: token },
    });
    paginatingExpenses();
  };
  ...
  const submitHandler = async (e) => {
    ...
    await axios.post("http://13.53.182.149:4000/expense/postAddExpense", obj, {
      headers: { Authorization: token },
    });
  ...
  const buyPremium = async () => {
    const response = await axios.get(
      "http://13.53.182.149:4000/purchase/buypremium",
      {
        headers: { Authorization: token },
      }
    );
    const rzp = new Razorpay({
      ...
      handler: async function (res) {
        ...
        const successMessage = await axios.post(
          "http://13.53.182.149:4000/purchase/updatePremiumStatus",
          {
            order_id: response.data.order.id,
    ...
    rzp.on("payment.failed", async (res) => {
      ...
      await axios.post(
        "http://13.53.182.149:4000/purchase/transactionFailed",
        { order_id: response.data.order.id },
        {
          headers: { Authorization: token },
        }
  ...
  const download = async () => {
    const res = await axios.get("http://13.53.182.149:4000/expense/download", {
      headers: { Authorization: token },
    });
    try {
      if (res.status === 200) {
